...
export class Utils {
    ...
    public static getPredDescriptions(pred: string): string {
        ...
        if (editor) {
            docTxt = editor.document.getText();
        }
        ...
        for (let key in Utils.snippets) {
            if (re.test(key)) {
                ...
                if (new RegExp("import\\s+.*\\b" + mod + "\\b").test(docTxt) ||
                    Utils.DEFAULTMODULES.indexOf(mod) > -1) {
                    descs += key.replace(":", ".") + ":" + Utils.snippets[key].description;
                }
            }
        }
        return descs;
    ...
    public static getPredicateUnderCursor(doc: TextDocument, position: Position): string {
